...
slugify(Str) when is_list(Str) ->
  binary_to_list(translit(unicode:characters_to_binary(Str), <<>>));
slugify(Str) when is_binary(Str) ->
  translit(Str, <<>>).

translit(<<>>, Acc) -> re:replace(Acc, "^-*|-*$", "", [global, {return, binary}]);
translit(<<C/utf8, T/binary>>, Acc) when C >= $A andalso C =< $Z ->
  translit(T, <<Acc/binary, (C + 32)/utf8>>);
translit(<<C/utf8, T/binary>>, Acc) when (C >= $a andalso C =< $z) orelse (C >= $0 andalso C =< $9) ->
  translit(T, <<Acc/binary, C/utf8>>);

translit(<<"-"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "-">>);
translit(<<"_"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "-">>);
translit(<<" "/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "-">>);
...
translit(<<"е"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "e">>);
translit(<<"Ё"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "oy">>);
translit(<<"ё"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "oy">>);
translit(<<"Ж"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hz">>);
translit(<<"ж"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hz">>);
...
translit(<<"ц"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "c">>);
translit(<<"Ч"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hc">>);
translit(<<"ч"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hc">>);
translit(<<"Ш"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hs">>);
translit(<<"ш"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hs">>);
translit(<<"Щ"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hhs">>);
translit(<<"щ"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "hhs">>);
...
translit(<<"э"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "he">>);
translit(<<"Ю"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "uy">>);
translit(<<"ю"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "uy">>);
translit(<<"Я"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "ay">>);
translit(<<"я"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "ay">>);
% Ukrainian support
...
translit(<<"і"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "i">>);
translit(<<"Є"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "ey">>);
translit(<<"є"/utf8, T/binary>>, Acc) -> translit(T, <<Acc/binary, "ey">>);
% Polish support
...
translit(<<_/utf8, T/binary>>, Acc) -> translit(T, Acc).
